...
as $$
    ...
    select
      case
        when js.file_count > 0
          and js.avg_expected_touches_raw is not null
          and js.avg_expected_touches_raw > 0
          and js.avg_expected_touches_raw <= 20
        then true
        else false
      end as has_expected_metrics,
      case
        when js.file_count > 0
  ...
  division_rows as (
    select
      ...
      case
        when m.has_expected_metrics then round(avg(j.expected_touches_raw), 2)::numeric
        else null
      end as avg_expected_touches,
      case
        when m.has_net_metrics then round(avg(j.net_touches_raw), 2)::numeric
        else null
      end as net_touches,
